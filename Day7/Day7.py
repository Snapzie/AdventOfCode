...
onePair = []
high = []
dict_ranking = {'J':0,'2':1,'3':2,'4':3,'5':4,'6':5,'7':6,'8':7,'9':8,'T':9,'Q':10,'K':11,'A':12}
...
for line in f.split('\n'):
    ...
    for c in hand:
        d[c] = d[c] + 1
    # vals = sorted(list(d.values()))[::-1]
    vals = sorted([d.get(k) for k in d.keys() if k != 'J'])[::-1]
    print(vals)
    if d['J'] == 5 or vals[0] + d['J'] == 5:
        fiveKind.append((hand,int(bid)))
    elif vals[0] + d['J'] == 4:
        fourKind.append((hand,int(bid)))
    elif vals[0] + d['J'] == 3 and vals[1] == 2:
        house.append((hand,int(bid)))
    elif vals[0] + d['J'] == 3:
        threeKind.append((hand,int(bid)))
    elif vals[0] == 2 and vals[1] == 2:
